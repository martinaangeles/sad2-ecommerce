...
function Customer() {
  ...
  <div className="flex items-center space-x-4">
    <label htmlFor="sortOption" className="w-20">Sort By:</label>
    <select
  ...
  <div>
    ...
    >
      ...
      <table className="flex flex-col basic mt-10">
        <thead>
          <tr className="grid grid-cols-8">
            <td className="truncate"> Customer ID </td>
            <td className="truncate"> Last Name </td>
            <td className="truncate"> First Name </td>
            <td className="truncate"> Email Address </td>
            <td className="truncate"> Username </td>
            <td className="flex justify-center truncate"> Status </td>
            <td className="truncate"> Rewards </td>
            <td className="truncate"> Actions </td>
          </tr>
        </thead>
        <tbody>
          {customer.map((data, i) => (
            <tr key={i} className="grid grid-cols-8">
              <td className="truncate">{data.customer_account_id}</td>
              <td className="truncate">{data.customer_account_firstName}</td>
              <td className="truncate">{data.customer_account_lastName}</td>
              <td className="truncate">{data.customer_account_emailAddress}</td>
              <td className="truncate">{data.customer_account_username}</td>
              <td className="flex justify-center truncate">
                {data.customer_account_status}
              </td>
              <td className="truncate">
                {rewardMapping[data.customer_account_reward_id_fk]}
              </td>
